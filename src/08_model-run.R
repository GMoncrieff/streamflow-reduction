...
sp_data_long <- read_feather('data/output/sp_data_long.feather') %>%
  select(-ID.y) %>%
  rename(ID=ID.x,agem = layer.1,agesd = layer.2)
...
if(!(uncer %in% c(8,0))){
  sp_data_long$agesd <- sp_data_long$agesd *0
}
...
pdat = list(M = nrow(sp_data_long), ID = sp_data_long$ID, NNUM = sp_data_long$niaps_rowid,
            ...
            age_sd = sp_data_long$agesd, age_m =  sp_data_long$agem,
...
foreach (j = 1:stansamp,
         ...
         .packages = c('dplyr',
           ...
           #age for each pixel
           x_a_p <- rnorm(pdat$M,pdat$age_m,pdat$age_sd) %>%
             pmax(0,.)
